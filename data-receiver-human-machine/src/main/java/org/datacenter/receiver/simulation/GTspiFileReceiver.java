...
import org.datacenter.model.simulation.GTspi;
...
/**
 * @author : [宁]
 * @description : 仿真数据GTspi的接收器
 */
@Slf4j
@Data
@EqualsAndHashCode(callSuper = true)
public class GTspiFileReceiver extends SimulationBaseReceiver<GTspi> {

    @Override
    public void prepare() {
        table = TiDBTable.GT_SPI;
        modelClass = GTspi.class;
        super.prepare();
    }
    ...
    protected SerializableFunction<CsvMapper, CsvSchema> getSchemaGenerator() {
        return mapper -> CsvSchema.builder()
                ...
                .addColumn("equipmentId")             // 装备ID
                .addColumn("status")                  // 状态
                .addColumn("northAngle")              // 指北角
                .addColumn("longitude")               // 经度
                .addColumn("latitude")                // 纬度
                .addColumn("altitude")                // 高度
                .addColumn("decoyPositionId")         // 假阵地编号
                .setUseHeader(true)
                .setColumnSeparator(',')
                .setLineSeparator("\n")
                .build();
